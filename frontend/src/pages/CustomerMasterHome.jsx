...
const CustomerMasterHome = () => {
  ...
  // Normalizing property names for pg case-insensitivity
    ...
    <div>
      ...
      <div className="card-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
        {customers.map(customer => {
          ...
          return (
            <div key={getProp(customer, 'custid', 'CustID')} className="module-card" style={{ display: 'flex', flexDirection: 'column', height: 'auto', minHeight: '120px', padding: '20px', cursor: 'default' }}>
        ...
        {customers.length === 0 && !loading && !error && <div>No customers found.</div>}
      </div>
    </div>
  );
};

export default CustomerMasterHome;
